...
-(void)setBookTitle:(UIView *)postContentView indexPath:(NSIndexPath *)indexPath{
    ...
    bookTitleButton * bookTitleBtn = [[bookTitleButton alloc]initWithFrame:CGRectMake(NAMELABEL_HEIGHT, NAMELABEL_HEIGHT, bookTitleLabel.frame.size.width, NAMELABEL_HEIGHT)];
    [bookTitleBtn setTitle:bookTitle forState:UIControlStateNormal];
    [bookTitleBtn setTitleColor:UIColorFromRGB(FINE_DARKGRAY) forState:UIControlStateNormal];
    [bookTitleBtn setIsbn:[NSString stringWithFormat:@"%@",[timelineRowData objectForKey:@"ISBN"]]];
    bookTitleBtn.showsTouchWhenHighlighted = YES;
    [bookTitleBtn addTarget:self action:@selector(bookTitleButtonTouchUp:) forControlEvents:UIControlEventTouchUpInside];
    bookTitleBtn.contentHorizontalAlignment = UIControlContentHorizontalAlignmentLeft;
    [postContentView addSubview:bookTitleBtn];
}

-(void)bookTitleButtonTouchUp:(bookTitleButton *)bookTitleBtn{
    UIStoryboard *storyboard = [UIStoryboard storyboardWithName:@"Main" bundle:nil];
    DetailBookViewController * controller = [storyboard instantiateViewControllerWithIdentifier:@"bookDetail"];
    controller.isbnFromOtherView = bookTitleBtn->isbn;
